...
官方 Claude Managed Agents 的事件类型表里有 `session.usage` 事件：会话**累计用量**快照，携带 usage 对象 + budget（无预算时 null）。**触发时机**：idle 转换时（每次 session 进入 idle 前发一个，预算暂停时发一个）——`https://platform.claude.com/docs/en/managed-agents/events-and-streaming.md`。
