...
func TestWorkerThreadStatusControlsSessionEvents(t *testing.T) {
	...
	if len(after.Data) != len(before.Data)+2 || mustSessionRecord(t, app, response.ID).Status != "idle" {
		t.Fatal("last child idle did not update session status")
	}
	scanner := bufio.NewScanner(stream.Body)
	for i, eventType := range []string{"session.thread_status_idle", "session.thread_status_idle"} {
	...
	if repeated := listSessionEvents(t, app, response.ID, "limit=100", defaultTestKey); !reflect.DeepEqual(repeated.Data, resumed.Data) {
		t.Fatal("old thread fact generated a new event on retry")
	}
...
func TestSessionIdlePreservesPendingChildActions(t *testing.T) {
	...
	assertIdle := func() {
		t.Helper()
		if mustSessionRecord(t, app, response.ID).Status != "idle" {
