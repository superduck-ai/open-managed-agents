...
关闭 drawer 会移除 query param；浏览器前进/后退通过 `popstate` 同步 UI 状态。
...
- dialog 保持打开并显示 `Version created successfully`。
...
- confirm 时先删除 custom versions，再删除 custom skill。
...
- Update success：version created success 状态。
