...
#339 / #346 保留组织成员校验、工作区归属与状态、Default 身份和保护、资源隔离、即时撤权、归档保护、Workspace Key 边界及共用权限设施。Billing 完整支持由基于 #346 的独立 PR 承接，包括自动继承、提权与恢复、角色变更、功能限制、前端空态和专项验收。成员管理 UI 属于 #336，邀请与组织切换属于 #338。

## 授权与数据

每次用户请求读取最新组织身份，再检查目标空间的组织归属、归档状态和有效成员关系，最后判断动作权限。权限只存在本次 Principal，不缓存到长期 session。普通空间的 Org Admin 继承管理权限，其他角色需要有效显式成员。工作区管理员不能取得组织管理权限。
...
旧版 Billing 在可访问空间中的资源与 Workbench 入口不按本次专项规则收窄。共用能力判断保留该行为；这不跳过最新组织身份、跨组织/跨资源归属、成员撤销和归档检查，也不授予组织管理权限。Default 统一按组织身份可用并忽略历史记录，`workspace_billing` 仅保留角色标识，不在普通空间生成继承关系。
