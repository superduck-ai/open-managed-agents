...
describe('Workload identity settings page', () => {
  test('creates, reviews, disables, restores, and deletes local preview providers', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Create provider' })).toBeTruthy());

    press(screen.getByRole('combobox', { name: 'Provider' }));
    await waitFor(() => expect(screen.getByRole('option', { name: 'GitHub Actions' })).toBeTruthy());
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Trust policy' })).toBeTruthy());
    expect((screen.getByDisplayValue('GitHub Actions') as HTMLInputElement).value).toBe('GitHub Actions');
    expect(screen.getByDisplayValue('https://token.actions.githubusercontent.com')).toBeTruthy();
    expect(screen.getByDisplayValue('repo:open-managed-agent/release:ref:refs/heads/main')).toBeTruthy();
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Disable provider?' })).toBeTruthy());
    ...
    await waitFor(() => expect(screen.getByText('No providers yet')).toBeTruthy());
    ...
    await waitFor(() => expect(screen.getByText('No disabled providers')).toBeTruthy());
