...
import { DeploymentAddSelectField, DeploymentSelectField, DeploymentTextArea, DeploymentTextField, LockedAgentReferenceField, ManagedSelectField, ManagedTextArea, ManagedTextField, VaultMultiSelect } from '../components/common';
import { entityDialogSubtitle, submitLabel } from '../labels';
import { type AgentApiResponse, type AgentPageResponse, type CredentialFormValues, type EntityOption, type EnvironmentApiResponse, type ManagedEntityApiResponse, type ManagedEntityFormValues, type ManagedEntitySection, type MemoryApiResponse, type MemoryFormValues, type MemoryStoreApiResponse, type PageResponse, type VaultApiResponse, type VaultCredentialApiResponse } from '../types';
import { errorMessage } from '../utils';
import { credentialFormValues, initialFormValues } from './model';

export function CredentialDialog({ credential, onClose, onSubmit }: { credential?: VaultCredentialApiResponse; onClose: () => void; onSubmit: (values: CredentialFormValues) => Promise<void> }) {
  const [values, setValues] = useState<CredentialFormValues>(() => credentialFormValues(credential));
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const canSubmit = values.displayName.trim() && (values.authType === 'static_bearer' ? values.mcpServerUrl.trim() && values.token.trim() : values.secretName.trim() && values.secretValue.trim());
      ...
      <DialogContent className="sm:max-w-[520px]">
        <form onSubmit={submit}>
          ...
          <div className="mt-5 space-y-4">
            <ManagedTextField label="Display name" value={values.displayName} onChange={(displayName) => setValues((current) => ({ ...current, displayName }))} autoFocus />
            <ManagedSelectField
              label="Auth type"
              ...
              onChange={(authType) => setValues((current) => ({ ...current, authType: authType === 'environment_variable' ? 'environment_variable' : 'static_bearer' }))}
            />
            {values.authType === 'static_bearer' ? (
              <>
                <ManagedTextField label="MCP server URL" value={values.mcpServerUrl} placeholder="https://example.com/mcp" onChange={(mcpServerUrl) => setValues((current) => ({ ...current, mcpServerUrl }))} />
                <ManagedTextField label="Token" value={values.token} placeholder="Token" onChange={(token) => setValues((current) => ({ ...current, token }))} />
              </>
            ) : (
              <>
                <ManagedTextField label="Secret name" value={values.secretName} placeholder="EXAMPLE_TOKEN" onChange={(secretName) => setValues((current) => ({ ...current, secretName }))} />
                <ManagedTextField label="Secret value" value={values.secretValue} placeholder="Secret value" onChange={(secretValue) => setValues((current) => ({ ...current, secretValue }))} />
              </>
            )}
          </div>
...
export function MemoryDialog({ memory, onClose, onSubmit }: { memory?: MemoryApiResponse; onClose: () => void; onSubmit: (values: MemoryFormValues) => Promise<void> }) {
  const [values, setValues] = useState<MemoryFormValues>(() => ({ path: memory?.path || '', content: memory?.content || '' }));
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[560px]">
        <form onSubmit={submit}>
          ...
          <div className="mt-5 space-y-4">
            <ManagedTextField label="Path" value={values.path} placeholder="/notes/example.txt" onChange={(path) => setValues((current) => ({ ...current, path }))} autoFocus />
            <ManagedTextArea label="Content" value={values.content} placeholder="Memory content" onChange={(content) => setValues((current) => ({ ...current, content }))} />
          </div>
          {error ? <p className="mt-4 text-sm text-destructive">{error}</p> : null}
          <DialogFooter className="mt-5">
            <Button type="button" variant="outline" onClick={onClose}>
...
export function ManagedEntityDialog({
    ...
    void (async () => {
      ...
      try {
        const [agentPage, environmentPage, vaultPage, memoryStorePage] = await Promise.all([
          ...
          section === 'deployments' ? listManagedEntities('memory-stores', workspaceId) : Promise.resolve({ data: [], next_page: null } as PageResponse<ManagedEntityApiResponse>)
        ]);
        if (!active) {
          return;
        }
        const agentOptions = lockedAgent
          ? [{ id: lockedAgent.id, label: lockedAgent.name || lockedAgent.id, secondary: `v${lockedAgent.version} · ${compactAgentId(lockedAgent.id)}` }]
  ...
  if (section === 'deployments') {
    return (
      <Dialog open onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="flex max-h-[min(760px,calc(100dvh-2rem))] flex-col sm:max-w-[560px]" showCloseButton={false}>
          <form
          ...
          >
            ...
            <div className="pr-8">
              ...
              <DialogDescription className="mt-1 text-sm leading-5 text-muted-foreground">
                {msg('managedAgents.deployments.dialogSubtitle', 'Deploy an agent with a trigger, environment, and credentials.')}
              </DialogDescription>
            </div>
            ...
            <div className="subtle-scrollbar mt-5 min-h-0 flex-1 space-y-[18px] overflow-y-auto pr-1">
              ...
              {lockedAgent ? (
                ...
                <DeploymentSelectField
                  ...
                  placeholder={loadingOptions ? msg('managedAgents.agents.loading', 'Loading agents...') : msg('managedAgents.deployments.selectAgent', 'Select an agent')}
                  options={agents}
              ...
              <DeploymentTextArea
                ...
                placeholder={msg('managedAgents.deployments.initialMessagePlaceholder', "Summarize today's support tickets and post to #digest")}
              ...
              <DeploymentSelectField
                ...
                placeholder={loadingOptions ? msg('managedAgents.environments.loading', 'Loading environments...') : msg('managedAgents.quickstart.selectEnvironment', 'Select an environment')}
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex max-h-[min(760px,calc(100dvh-2rem))] flex-col sm:max-w-[560px]" showCloseButton={false}>
        ...
        <div className="pr-8">
          ...
          {dialogSubtitleText ? <DialogDescription className="mt-1 text-sm leading-5 text-muted-foreground">{dialogSubtitleText}</DialogDescription> : null}
        </div>

        <div className="subtle-scrollbar mt-5 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
          <ManagedTextField
            label={section === 'sessions' ? msg('managedAgents.sessions.fieldTitle', 'Title') : msg('common.name', 'Name')}
            value={values.name}
            placeholder={section === 'sessions' ? msg('managedAgents.sessions.titlePlaceholder', 'Optional - name this run') : msg('managedAgents.common.namePlaceholder', 'Enter a name')}
            onChange={(name) => setValues((current) => ({ ...current, name }))}
            autoFocus
          />
          ...
          {section === 'environments' ? (
            <>
              <ManagedTextField label={msg('managedAgents.environments.hostingType', 'Hosting type')} value={msg('managedAgents.environments.cloud', 'Cloud')} disabled onChange={() => undefined} />
              <ManagedTextArea
          ...
          {needsReferences ? (
            <>
              ...
              />
              <Collapsible
                open={resourceDisclosureOpen}
                onOpenChange={setResourceDisclosureOpen}
              >
                  ...
                  <CollapsibleContent className="border-t border-border">
                    <CardContent className="px-3 pb-3 pt-2">
                      <p className="text-sm leading-5 text-muted-foreground">
                        {msg('managedAgents.common.noResourceAttachments', 'No resource attachments are configured. Add files, repositories, or memory stores after creation.')}
                      </p>
                    </CardContent>
                  </CollapsibleContent>
                </Card>
          ...
          {section === 'credential-vaults' ? (
            <p className="text-sm leading-5 text-muted-foreground">
              {msg('managedAgents.credentialVaults.createHint', 'Continue after creating the vault to add credentials for tools and MCP servers.')}
            </p>
          ) : null}
        </div>
