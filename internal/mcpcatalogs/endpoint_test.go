...
func TestNormalizeEndpointPreservesQueryAsCatalogIdentity(t *testing.T) {
	got, err := NormalizeEndpoint(" HTTPS://Example.COM.:443/mcp?toolsets=all%2Cvisualizations ")
	if err != nil {
		t.Fatalf("NormalizeEndpoint returned error: %v", err)
	}
	if want := "https://example.com/mcp?toolsets=all%2Cvisualizations"; got != want {
