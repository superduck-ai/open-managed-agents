...
Agent Detail 中的 MCP 工具清单不能只依赖静态 MCP Directory，也不应在创建 Agent 时固化到 `agents` 或 `agent_versions`。MCP 的 `tools/list` 是外部服务在特定时间、网络与凭据上下文下返回的可变观测结果，不属于 Agent 不可变配置。
...
3. 用户可以在详情页手动刷新。手动刷新绕过 freshness TTL，但不会绕过安全策略，也不会为同一 endpoint 创建并行重复任务。
4. 运行时可以在未来把带凭据的 `tools/list` 观测结果写回相同 catalog 体系，但不同凭据作用域之间不合并、不互相复用。
5. 浏览器不直接连接 MCP endpoint；所有网络访问均由受控 worker 或运行环境执行。
...
| 浏览器直接调用 MCP | 后端实现较少 | CORS、凭据泄露和网络边界不可控，无法形成统一租户审计与限流 |
| **异步预热 + 持久 cache + 按需刷新** | Agent 写入可靠；首次延迟较低；支持 stale 展示、自愈、去重和安全控制 | 需要 catalog、job 和状态机；本设计选择此方案 |
...
这说明当前页面空态的直接原因是缺少动态发现链路，而不是 Weather MCP 没有工具。之前使用 `host.docker.internal` 时存在网络可达性差异；新 `.local` 地址已解决当前环境的连通性，但生产实现仍必须保留 SSRF、DNS 和出口策略。
...
| Discovered catalog | 某 endpoint 在某一 auth scope 下最近一次成功 `tools/list` 的结果 | 否 |
| Runtime observation | Session/Environment 使用实际凭据观测到的工具清单 | 否 |
| Auth scope | 决定可见工具集合的匿名或凭据上下文标识；只保存不可逆标识，不保存 token | 否 |
...
- 多个 Agent 可以在同一 workspace 内复用同一 endpoint/auth scope 的 catalog。
...
- 不根据 Agent Detail 自动选择 vault 或跨凭据 scope 合并工具。
...
6. endpoint cache 的租户边界至少是 `organization_id + workspace_id`；所有读取、写入、清理与调度必须显式带两者。
7. 不同 auth scope 的 catalog 必须隔离；匿名 catalog 不能代表带凭据运行时的完整能力，反之亦然。
...
flowchart LR
  ...
  Worker --> Policy["Egress + SSRF policy"]
  Policy --> MCP["MCP endpoint"]
  MCP --> Worker
  Worker --> CatalogDB
  Runtime["Future runtime observation"] --> CatalogDB
...
Catalog 主键不包含 Agent ID 或 Agent version，而是 tenant、endpoint 和 auth scope：

`(organization_id, workspace_id, endpoint_key, auth_scope_key)`

Agent Detail 请求中的 Agent/version 只用于：

1. 在对应 workspace 内读取该版本的 `mcp_servers`；
2. 为每个 server 计算 endpoint key；
3. 找到可复用的 catalog；
4. 使用该版本的权限配置生成展示结果。
...
server 仅改名但 transport 和 URL 不变时可以复用 catalog；URL 或 transport 改变会生成新的 endpoint key，旧任务和旧结果不得覆盖新 endpoint。

## 8. 持久化模型

实现时新增 goose migration：

`internal/db/migrations/00013_add_mcp_tool_catalogs.sql`

`00013` 是本文编写时的下一个编号；若落地前已有新 migration，应使用当时的下一个编号。不得修改已应用 migration 或 `internal/db/schema.go`，且不得创建 PostgreSQL foreign key。
...
| `organization_id` | `bigint not null` | 显式租户 scope，无 FK |
| `workspace_id` | `bigint not null` | 显式 workspace scope，无 FK |
| `transport_type` | `text not null` | 当前为 URL transport；为以后 transport 扩展保留 |
| `endpoint_url` | `text not null` | 不含凭据的完整可请求 URL；不返回给 catalog API，不写入普通日志 |
| `endpoint_key` | `text not null` | transport + 完整规范化 URL 的服务端 HMAC |
| `auth_scope_key` | `text not null` | MVP 为 `anonymous`；未来由 tenant、凭据引用与凭据版本生成 HMAC |
| `auth_scope_reference` | `text null` | 服务端可解析的非 secret 凭据上下文引用；anonymous 为 `NULL`，不返回 API |
| `tools` | `jsonb null` | `NULL`=从未成功，`[]`=真实空列表 |
| `source` | `text null` | `anonymous_probe` 或 `runtime_observation` |
...
| `last_referenced_at` | `timestamptz not null` | create/update 预热、Detail 映射或 runtime observation 最近引用时间 |
| `created_at` | `timestamptz not null` | 创建时间 |
| `updated_at` | `timestamptz not null` | 更新时间 |

约束与索引：

- unique `(organization_id, workspace_id, endpoint_key, auth_scope_key)`；
- index `(organization_id, workspace_id, expires_at)`；
- index `(organization_id, workspace_id, last_referenced_at)`，供 tenant-scoped retention 扫描；
...
`endpoint_key` 必须覆盖 transport 和完整可请求 URL，因为 query 也可能改变服务端暴露的工具集合。规范化至少包括 scheme/host 大小写、默认端口和 fragment 处理；path 与 query 的语义必须保留。使用服务端 HMAC 而不是把 raw URL 用作索引或前端 query key，避免把可能含敏感 query 的 URL 暴露在日志和开发工具中。

`endpoint_url` 与 Agent 配置处于同一数据库安全边界，供 worker 请求使用，但它仍必须是 credential-free URL。anonymous MVP 默认拒绝带 query 的 endpoint；只有服务端明确允许的非敏感配置参数（例如 Directory 已知的 `toolsets`）可以保留。`token`、`key`、`secret`、`signature`、`authorization` 等敏感名称无论大小写都必须拒绝，不能只靠名称 denylist 证明未知参数安全；未知 query 不得静默剥离后探测，因为这会改变 endpoint 语义。URL userinfo 同样始终拒绝。

未来 authenticated scope 仍使用同一 credential-free `endpoint_url`。授权上下文只保存为独立的 `auth_scope_reference`，由 worker 在租户鉴权后解析 vault/session credential 并在出站时注入；`auth_scope_key` 由 `organization_id + workspace_id + auth_scope_reference + credential version` 的服务端 HMAC 派生。原始 token、header、cookie 或 secret query 不得进入 catalog、Agent 派生字段、job payload、日志或 API。
...
- `last_referenced_at` 超过 30 天且没有 active generation 的 catalog 整行删除，同时删除只引用该 catalog 的终态 discovery jobs。Agent create/update 预热、Detail 映射和 runtime observation 都必须刷新该字段；因此 endpoint churn、长期未访问的历史版本和废弃 auth scope 会自然回收，之后访问仍可按 Agent 配置重建。
- credential update/archive/delete 立即使旧 `auth_scope_reference` 对应 catalog 失效；凭据删除后清理行时不需要也不能读取原始 secret。
- workspace 或 organization 删除必须在应用层删除事务中按 `organization_id + workspace_id` 显式删除 catalog 和关联 discovery jobs，再删除 tenant 资源。由于没有 FK，缺少该清理步骤必须导致删除事务失败，而不是遗留记录。

后台 retention job 每日至少运行一次，按 `organization_id + workspace_id` 小批量删除并复用 generation/lease 保护，不能删除 active job 正在写入的行。测试必须覆盖 hard-stale 清空不是空数组、endpoint churn 回收、active generation 保护、凭据删除，以及 workspace/organization 删除后零残留。
...
| `auth_required` | 无 last-good tools，最近终态是下游 401/403 | 当前 scope 需要认证 |
...
stateDiagram-v2
  ...
  AuthRequired --> Loading: retry with valid scope
  Error --> Loading: retry
```

初始建议为：动态结果 fresh TTL 10 分钟，Directory metadata TTL 1 小时。过期的 last-good catalog 在 30 天 hard-stale 截止时间前继续以 stale 状态展示，必须同时显示发现时间和刷新错误；它不会参与运行时授权或工具执行。hard-stale 清理按 8.3 执行，不能通过把 `tools` 改成 `[]` 来表示过期。fresh TTL 应加入少量 jitter，防止同一时间集中刷新。
...
| Credential update/archive | 使对应 auth scope 失效或调度新 scope，不能污染 anonymous scope |
| Runtime `tools/list_changed` | 使对应 runtime scope 过期并 ensure；短连接 MVP 可后续实现 |
| Backfill | 对现有 Agent 分批入队；migration 本身绝不访问外部网络 |

Agent create/update 的预热沿用现有异步预热模式：Agent 事务成功后再 best-effort 调度。调度失败只记录脱敏日志，不改变已经成功的 Agent 响应。Agent Detail 的 ensure 是漏单后的自愈路径；除非实现引入真正的 outbox，否则不得声称 Agent 写入和预热任务原子提交。
...
payload 不保存 token、header、vault secret、响应体，尽量不复制 URL。`trigger` 允许 `agent_create`、`agent_update`、`detail_read`、`manual_refresh`、`runtime_list_changed` 和 `backfill`。
...
job external ID 使用稳定 hash，例如：

`job_mcpt_<sha256(type, organization_id, workspace_id, catalog_external_id, generation) 前 40 位>`

插入使用 `ON CONFLICT (external_id) DO NOTHING`。同一 generation 的 transient failure 只把同一 job 从 `running` 转为 `retry`，不创建新 generation。

worker lease、`FOR UPDATE SKIP LOCKED`、`locked_by/locked_until`、owner 校验和 retry 语义沿用现有 skill prewarm job。写结果时使用 generation CAS：只有 job generation 仍等于 catalog 的 `requested_generation` 才能更新；过期 job 完成但不写结果，防止旧请求覆盖新 endpoint 或新刷新。
...
- 同一 `(organization_id, workspace_id, endpoint_key, auth_scope_key)` 同时最多一个 active generation。
- 全局、workspace 和 endpoint 分别设置并发/速率限制。
...
1. 根据受支持 transport 创建客户端；URL MCP 优先使用 Streamable HTTP，兼容明确配置或已识别的 legacy SSE endpoint。
...
短生命周期匿名 probe 不依赖 `notifications/tools/list_changed`。若 Session/Environment 持有长连接并收到该通知，运行时可将对应 scope 标为 stale 并调度刷新。

## 12. Console API

Catalog 是 Console 展示能力，不属于 Anthropic 兼容 API。拟新增受 `platformAuthMiddleware` 保护的路由：
...
注册器拟命名为 `platformapi.RegisterConsoleOrganizationAgentRoutes`，由 `internal/api/server.go` 在现有 `/api/console/organizations/{orgUuid}` 鉴权 group 中挂载。`internal/api` 只做组装；查询校验、状态映射和调度编排放在聚焦的 MCP catalog 资源包中；SQL 仅放 `internal/db`。
...
- DB 查询同时使用 principal 的内部 organization ID 和 workspace ID，而不是只相信 path 或 `agentId`；
- 在该 workspace 内读取 Agent/version，缺失返回 404；
- 从 Agent 版本服务端解析 MCP URL，刷新 API 不接受调用方提供任意 URL、header 或 token。

### 12.1 GET catalog

GET 立即返回 cache，不等待外部 MCP。对于 missing/stale catalog，可以在响应过程中做幂等 ensure，但网络 probe 始终异步执行。

```json
{
  "agent_id": "agent_PN9kQ4b1ZFgWrOu8X9Wz01KE",
  "version": 2,
  "catalogs": [
    {
      "server_name": "weather_service",
      "endpoint_fingerprint": "mcpe_7f1b...",
      "status": "ready",
...
`status` 是 freshness/error 展示语义的唯一权威字段；响应不再额外返回可与其冲突的 `stale` boolean。`refresh.active` 只描述调度是否仍在执行，不替代 `status`。
...
已有 active generation 时返回相同 generation 和 `queued:false`。手动 `force` 可以绕过 freshness TTL 和自动刷新使用的短期负缓存，以便用户主动重试；它不能绕过 active generation、workspace 限流、SSRF、出口或认证策略。路由遵循现有平台 session 的 CSRF/同源防护约定。
...
```text
Directory key:
['console', 'mcp-directory', 'remote-commercial', schemaVersion]

Catalog key:
['console', 'agent-mcp-tool-catalogs', orgUuid, workspaceId, agent.id, version]
```

query key 不包含 raw URL。`orgUuid` 不能省略，因为不同 organization 都可能存在 `default` workspace。Agent/version 改变时进入新 key，不使用跨 key 的 `keepPreviousData`，避免旧版本工具短暂闪现。

建议参数：

- Directory：`staleTime=1h`、`gcTime≈24h`；
- Catalog：`staleTime` 不长于服务端 TTL，初始 5–10 分钟；`gcTime≈1h`；
- `retry=false`、`refetchOnWindowFocus=false`、`refetchOnReconnect=false`，避免聚焦页面时触发外部调度；
- 把 `AbortSignal` 传给 fetch，版本切换和卸载时可取消；
- Query cache 只作为内存展示缓存，不持久化；登出、账号/org 切换时清理 catalog queries。

当 GET 返回 `loading` 或 `refreshing` 时，前端只轮询 cache-only GET，不发起重复 refresh。轮询采用短期退避并设置总时限；服务端 generation/singleflight 仍是 StrictMode 和多标签页下的最终去重边界。

手动 Refresh 使用 mutation 调用 POST，成功后 invalidates 同一个 catalog query；失败不能用错误响应覆盖现有工具。按钮 busy 时 disabled，重复点击不会新增 generation。
...
| auth_required | `—` | 当前 scope 需要认证；不得暗示 Agent 无工具 |
| error | `—` | 稳定错误文案和 Retry |
...
MCP URL 是用户可控的出站目标，动态发现属于 SSRF 敏感能力。实现必须在网络请求前和每次 redirect/dial 时执行策略：

- 只允许明确支持的 `http`/`https` scheme，拒绝 URL userinfo、fragment 和非法端口；
- anonymous probe 默认拒绝 query；仅允许服务端配置的非敏感 query key/value，并始终拒绝 credential-like key。不得通过删除未知 query 后继续请求；
- 解析 DNS 后校验每个目标 IP，并防御解析后换址；
- 默认拒绝 metadata、link-local 和未授权的 loopback/private 网段；
- private/VPC/`.local` endpoint 通过部署级可信 allowlist 或与 Session 相同的受控 environment worker 访问，而不是由前端绕过策略；
- redirect 每一跳重新解析与校验，默认同 origin，跨 origin 必须显式允许；
- 出站请求不继承 Console 用户 cookie、平台 header 或任意环境代理凭据；
- endpoint/workspace 维度限流，限制响应页数、字节数和字段长度；
- 日志仅记录 workspace、catalog external ID、endpoint fingerprint、generation、duration 和稳定 error code；
- 原始 URL query、token、Authorization、vault ID 组合、响应体和堆栈不得进入可见日志或 API。

当前验收域名 `arthurs-MacBook-Pro-2.local:39090` 需要在本地开发/测试出口策略中显式允许。生产默认策略不能因为本地用例可达而开放任意内网地址。

Agent Detail 没有 Session vault context，因此 MVP 只自动读取/刷新 `anonymous` scope。若匿名 probe 返回 401/403，显示 `auth_required`。未来带凭据发现必须由已授权的 Session/Deployment/Environment context 触发：catalog 只保存可在同一 tenant 内解析的 `auth_scope_reference`，`auth_scope_key` 使用 organization、workspace、credential reference、版本和可见 scope 的 HMAC，绝不包含 token，也不能把一个用户的 catalog 暴露给另一个 scope。
...
| MCP 401/403 | anonymous scope 写 `auth_required`；不自动猜 vault |
| SSRF 策略拒绝 | 写 `blocked_target`；不发网络请求 |
| response/schema 无效 | 写 `invalid_response`；保留 last-good |
| worker lease 丢失 | 由现有 lease recovery 重领；CAS 防止旧 worker 覆盖 |
| URL 在刷新期间改变 | 新 endpoint key/new generation；旧 job 完成但 no-op |
...
- discovery attempts、success、auth_required、blocked、timeout、invalid_response；
...
- workspace/global active jobs 与 backlog age；
...
- `internal/db/mcp_tool_catalogs.go`：organization/workspace-scoped catalog CRUD、generation CAS、retention 与 job lease/transition。
- `internal/db/migrations/00013_add_mcp_tool_catalogs.sql`：新表与约束，不创建 FK。
...
1. 新增 migration、organization/workspace-scoped DB catalog CRUD、generation、retention 和 job 数据访问。
2. 实现匿名 URL probe、SSRF/egress policy、worker lease/retry/CAS。
3. 在 Agent create/update 成功后加入 best-effort ensure。
4. 新增受保护 Console GET/refresh API，保持 `/v1` 不变。
5. 前端接入 TanStack Query、状态机、manual refresh 和 a11y 文案。
6. 对现有 Agent 分批 enqueue，或依赖 Detail GET lazy ensure；migration 不做网络 backfill。
7. 使用 feature flag/配置逐步开启自动发现，保留 Directory fallback。
8. 后续增加 runtime authenticated observation 和 `tools/list_changed` 失效链路。

实现完成后，应把本文状态改为 Implemented，并同步更新当前展示基线文档中的数据流、缓存与空态描述；在此之前，基线文档继续准确描述现有静态 Directory 行为。
...
- SSRF blocked target、DNS rebinding、redirect 越界和 URL 脱敏；
...
- organization/workspace 复合隔离、相同 URL 跨 tenant 不共享、同 tenant endpoint 复用；
...
- anonymous query allowlist、auth scope reference、hard-stale/endpoint churn 清理，以及 workspace/organization 删除零残留；
...
- initial timeout/auth/blocked 文案；有旧数据时刷新失败保留列表并进入 stale；
...
7. `tools/list_changed` 在实现 runtime observation 后能使对应 scope 失效。
