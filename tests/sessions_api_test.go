...
func TestCreateSessionResourceValidationIsSerialized(t *testing.T) {
	app := newTestAppWithStore(t, nil, newFakeStore("sessions-resource-serialization-bucket"))
	defer app.close()

	agent := createAgent(t, app, `{"model":"claude-opus-4-6","name":"session-resource-serialization-agent"}`)
	defer cleanupAgentRows(t, app.db, agent.ID)
	env := createEnvironment(t, app, `{"name":"session-resource-serialization-env"}`)
	defer cleanupEnvironmentRows(t, app.db, env.ID)
	created := createSession(t, app, `{"agent":`+quoteJSON(agent.ID)+`,"environment_id":`+quoteJSON(env.ID)+`}`)
	defer deleteSession(t, app, created.ID)

	session, err := app.db.GetSession(context.Background(), getDefaultDBIDs(t, app.db).WorkspaceID, created.ID)
	if err != nil {
		t.Fatalf("load session: %v", err)
	}
	newResource := func(suffix string) db.SessionResource {
		externalID := "sesrsc_" + strings.ReplaceAll(uuid.NewString(), "-", "")
		return db.SessionResource{
			...
			Payload: json.RawMessage(
				`{"id":` + quoteJSON(externalID) +
					`,"type":"memory_store","memory_store_id":"mem_serialized_` + suffix + `"}`,
			),
	...
	go func() {
		_, createErr := app.db.CreateSessionResource(ctx, newResource("first"), nil, func([]db.SessionResource) error {
			close(firstEntered)
			<-releaseFirst
			return nil
		})
	...
	case <-firstEntered:
	case <-ctx.Done():
		t.Fatalf("first validator did not run: %v", ctx.Err())
	}

	secondEntered := make(chan []db.SessionResource, 1)
	secondResult := make(chan error, 1)
	validationFailure := errors.New("second resource rejected after observing first")
	go func() {
		_, createErr := app.db.CreateSessionResource(ctx, newResource("second"), nil, func(resources []db.SessionResource) error {
			secondEntered <- append([]db.SessionResource(nil), resources...)
	...
	select {
	case <-secondEntered:
		t.Fatal("second validator ran before the first resource transaction released its Session lock")
	...
	var secondSnapshot []db.SessionResource
	select {
	case secondSnapshot = <-secondEntered:
	case <-ctx.Done():
		t.Fatalf("second validator did not run after lock release: %v", ctx.Err())
	}
	if err := <-secondResult; !errors.Is(err, validationFailure) {
		t.Fatalf("create second resource error = %v, want validation failure", err)
	}
	if len(secondSnapshot) != 2 {
		t.Fatalf("second validator saw %d resources, want persisted first plus pending second", len(secondSnapshot))
	}
	resources, err := app.db.ListSessionResources(ctx, session.WorkspaceID, session.ExternalID)
	if err != nil {
		t.Fatalf("list serialized resources: %v", err)
	}
	if len(resources) != 1 {
		t.Fatalf("persisted resources = %d, want only the first", len(resources))
	}
}
...
func assertSessionFileReference(
	...
	if entry.Kind != db.FilestoreEntryKindFile ||
		...
		*entry.ManagedBy != "session_file_resource" ||
		entry.ManagedResourceExternalID == nil ||
		*entry.ManagedResourceExternalID != payload.ID ||
		entry.MD5 != nil ||
		entry.ExpiresAt != nil ||
		entry.SizeBytes == nil ||
