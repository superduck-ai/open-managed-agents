...
3. 后端从已鉴权的 Agent 版本读取 endpoint；普通远程 MCP 通过匿名 Streamable HTTP 调用
   `tools/list`，OMA Tunnel URL 则验证当前 workspace 归属后通过 NATS Broker 和 connector 探测。
...
2. 复用 `internal/tunnels.Service.Probe`，通过 NATS Broker 把 `initialize`、`notifications/initialized` 和
