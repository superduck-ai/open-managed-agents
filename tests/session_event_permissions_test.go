...
func TestWorkerPermissionCommitsEventsAndMetadataTogether(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil || string(after.WorkerExternalMetadata) != string(worker.WorkerExternalMetadata) {
			...
			if tc.policy == "always_ask" {
				...
				if countQueuedCodeSessionInboundEvents(app, codeSessionID, "control_response", "request-permission") != 0 {
			...
			if err != nil || countQueuedCodeSessionInboundEvents(app, codeSessionID, "control_response", "request-permission") != 1 {
			...
			after, err = getCodeSession(app, t.Context(), codeSessionID)
			if err != nil || countQueuedCodeSessionInboundEvents(app, codeSessionID, "control_response", "request-permission") != 1 || string(after.WorkerExternalMetadata) != string(committed.WorkerExternalMetadata) {
			...
			count := countQueuedCodeSessionInboundEvents(app, codeSessionID, "control_response", "9007199254740993")
			if count != 1 {
