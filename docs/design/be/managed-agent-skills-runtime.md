...
1. 从 session 的 agent snapshot 读取 `skills[]` 与 MCP server URL，并在 runner 内存中的 environment work 上准备 managed-agent metadata；该步骤不提前持久化 Environment Work。
2. 从可被 `net/url` 解析且包含 hostname 的 MCP URL 提取小写 hostname，与 Environment networking allowlist 合并并去重。当前 scheme 不参与筛选；只有 hostname 会成为 egress 规则，URL scheme、port、path、query、fragment 和 userinfo/credential 本身都不会写入 allowlist。
...
8. 把 mount 信息继续写入内存中的 environment work metadata；随后才调用 runtime provider `Resolve`，使准备好的 MCP hosts 参与本次 limited-network `AllowOut` 计算。
9. 用 `Resolve` 的结果和同一份内存 work 调用 sandbox `Create`，使 skill volume 在创建时挂载到 `/mnt/skills`。heartbeat 确认 Work 仍可运行后，runner 才以单个事务创建 local code session/events，并持久化 Work/Session 的 preparation metadata 与 runtime identity；事务提交后写入 `environment-manager` stdin 并启动 environment-manager。runner 不再把 skill zip 写进 sandbox 临时目录，也不在启动 shell 中解压 skill。
...
MCP 网络策略与上述 preparation/Resolve 顺序使用同一合同：runner 从 session 固定的 agent snapshot 读取 MCP server URL，通过 `net/url` 解析并提取非空 hostname 写入内存 metadata；provider `Resolve` 在 `allow_mcp_servers: true` 时将这些 hostname 与 Environment 的 `networking.allowed_hosts` 合并、规范化并去重，使它们进入本次 limited-network `AllowOut`。当前实现不按 URL scheme 过滤，因此任何可解析且含 hostname 的 MCP URL 都可能贡献一个 egress hostname；scheme、port、path、query、fragment 和 userinfo/credential 不会作为独立 allowlist 内容传播。因为 agent snapshot 在 session 创建时已经固定，后续 agent 更新不会改变已启动或正在创建的 session egress。

`environment_sandboxes.metadata` 的语义是一次 Sandbox 创建尝试的输入快照。它可以保留 `mcp_allowed_hosts`、skill manifest/hash/volume/mount 等 preparation 信息，即使创建或后续 provisioning 失败，也用于审计与诊断；这不表示 runtime commit 已成功。“heartbeat 成功后持久化”只约束 Environment Work/Session 上的 preparation state 与 `claude_code_*`/`runtime` identity：这些字段必须在 runtime commit 事务中一起成功，失败时不得遗留部分 identity。
...
flowchart TD
  ...
  A["Agent snapshot contains skills[] and MCP URLs"] --> B["Environment runner loads session snapshot"]
  B --> BA["Extract controlled MCP hosts into in-memory work metadata"]
  BA --> C["Resolve skill refs by workspace scope"]
  C --> D{"Skill source"}
  ...
  K --> M["Patch in-memory work metadata with managed_agent_skills_mount"]
  LA --> M
  M --> MA["Resolve template and network including MCP hosts"]
  MA --> N["Create sandbox with resolved network and /mnt/skills volume mount"]
  N --> NA["Heartbeat; transactionally commit Work/Session runtime identity"]
