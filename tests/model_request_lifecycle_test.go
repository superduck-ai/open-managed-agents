...
func TestMessagesProxyRequestLifecycle(t *testing.T) {
	...
	t.Run("history preserves insertion order at equal processed times", func(t *testing.T) {
		...
		params.Cursor = &db.SessionEventPageCursor{ProcessedAt: first[1].ProcessedAt, ExternalID: first[1].ExternalID}
		...
		if err != nil || len(filtered) != 3 {
			t.Fatalf("created_at filter must compare processed_at: %+v %v", filtered, err)
		}
