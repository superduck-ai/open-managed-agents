...
- 内置工具回显当前固定 Claude Code 2.1.120 的 22 项可选工具，与创建页、Agent API 合同和后端 `--tools` 派生清单一致；未显式配置时 `ask_user_question` 默认关闭，用户可以主动开启。`web_fetch` 对应 Claude Code 本地 `WebFetch`，不启用 Messages API 的模型服务端同名工具。内置 `web_search` 已永久移除，不在 Rendered 或 Raw 合同中。
