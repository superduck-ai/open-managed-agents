...
3. 对 `type=anthropic`，从 built-in catalog 读取 archive 元数据。
...
`NewRunnerWithConfig` 会创建 runtime resolver，即使没有 object store 也可以解析 built-in skill 元数据。custom skill 需要对象存储读取 archive；如果 managed agent session snapshot 包含 custom skill，而 runner 未配置 object store，runner 必须显式失败并停止该 work，不能继续创建缺失 `/mnt/skills` 的 sandbox。没有 skills 的 session 仍可通过旧 runner 构造入口正常启动。
