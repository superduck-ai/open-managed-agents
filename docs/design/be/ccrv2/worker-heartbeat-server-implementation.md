...
- 保持当前 worker ingress token-only 鉴权模式，为后续 JWT 验签保留替换空间。
...
Authorization: Bearer {code_session_id}
...
当前实现沿用 worker ingress token-only 模式：

```go
Authorization: Bearer {code_session_id}
```

`authorizeIngress` 只检查 bearer token 是否等于 path 中的 `code_session_id`。本次没有实现真实 JWT 签名验证、过期校验或 claim 校验。

后续如果替换为 JWT，应保持 handler 的外层调用顺序不变：
...
2. 调用 `authorizeIngress` 校验 worker ingress token。
...
过期 heartbeat 不会 bump epoch。下一次 worker 通过 register 或 bridge 注册时，仍由注册流程递增到下一个 epoch。
...
heartbeat 使用 `SELECT ... FOR UPDATE` 锁定目标 `code_sessions` 行，因此与 register/bridge 的 epoch bump、其他 heartbeat 更新在同一行上串行化。

该设计保证：

- register/bridge 先提交时，旧 epoch heartbeat 看到新 epoch 并返回 `409`。
- heartbeat 先提交时，只会续当前 epoch 的租约；后续 register/bridge 仍会 bump 到下一 epoch。
...
服务端的 `410` 表示该 epoch 的租约已经超过 grace，不会靠后续同 epoch heartbeat 恢复。真正恢复应通过 register/bridge 获取新 epoch。
...
- 过期 heartbeat 不 bump epoch，下一次 register/bridge 继续递增到下一 epoch。
...
```bash
go test ./tests -run 'TestCodeSessionWorker.*Heartbeat|TestCodeSessionWorkerEpochProtection|TestCodeSessionBridgeBumpsWorkerEpoch|TestCodeSessionWorkerEpochZeroRejectedAtDBLayer|TestCodeSessionWorkerEpochValidationRejectsInvalidValues' -count=1
