...
sidebarTitle: "Session operations"
description: "Retrieve, list, update, archive, and delete Open Managed Agents sessions."
---

Once a session exists, use these operations to read, update, archive, or delete it. See [Start a session](/docs/en/sessions) for creating a session and sending it work.
...
Sessions progress through these statuses. See [Start a session](/docs/en/sessions) for the session lifecycle.
...
Only the agent's `tools` and `mcp_servers` can change after a session is created. To run a session with `model`, `system`, or `skills` values other than the agent's, use [agent configuration overrides](/docs/en/sessions#override-agent-configuration-for-a-session) when you create the session. The agent's model configuration, including its `inference_geo` pin, also can't change mid-session: set the pin when you save the agent, or set or clear it for a single session with a `model` override when you create it. The agent's configured `system` field is fixed for the session's lifetime. On models that support it, you can still append system-level guidance mid-session by sending a [`system.message` event](/docs/en/events-and-streaming#sending-system-messages).
...
The session must be `idle` to update the agent. [Interrupt](/docs/en/events-and-streaming#integrating-events) the session if you need to update the agent while it's running.
...
A session [created with a budget](/docs/en/sessions#set-a-session-budget) accepts two kinds of budget update: replacing the cap with a new `max_list_cost`, and removing it by setting `budget` to `null`. Both automatically resume work that paused when the session reached its cap. A replacement cap can be higher or lower than the current one, but it must be strictly greater than the session's consumed list cost, and removal is one-way: a non-null `budget` is accepted only on a session that currently has one, so you can't re-add a removed budget or add one to a session created without it. See [Session budgets](/docs/en/budgets#resume-a-session-at-its-budget) for request examples, the error behaviors, and what counts toward list cost.
...
Archive a session to prevent new events from being sent while preserving its history. A `running` session cannot be archived; send an [interrupt event](/docs/en/events-and-streaming#integrating-events) if you need to archive it immediately.
...
Delete a session to permanently remove its record, events, and associated sandbox. A `running` session cannot be deleted; send an [interrupt event](/docs/en/events-and-streaming#integrating-events) if you need to delete it immediately.
