...
6. endpoint cache 的租户边界至少是 `organization_id + workspace_id`；所有读取与写入必须显式带 workspace scope。
...
Catalog 主键不包含 Agent ID 或 Agent version，而是 endpoint 和 auth scope：

`(workspace_id, endpoint_key, auth_scope_key)`
...
| `endpoint_url` | `text not null` | 完整可请求 URL；不返回给 catalog API，不写入普通日志 |
| `endpoint_key` | `text not null` | transport + 完整规范化 URL 的服务端 HMAC |
| `auth_scope_key` | `text not null` | MVP 为 `anonymous`；未来为凭据上下文 HMAC |
...
- unique `(workspace_id, endpoint_key, auth_scope_key)`；
...
`endpoint_url` 与 Agent 配置处于同一数据库安全边界，供 worker 请求使用。实现应拒绝 URL userinfo，避免在 URL 中携带凭据，并在错误、job payload、metrics 和结构化日志中始终使用 endpoint key 或脱敏 origin。
...
初始建议为：动态结果 fresh TTL 10 分钟，Directory metadata TTL 1 小时。过期的 last-good catalog 继续以 stale 状态展示，必须同时显示发现时间和刷新错误；它不会参与运行时授权或工具执行。数据保留与是否增加 hard-stale 截止时间可以后续配置化，不能通过把 `tools` 改成 `[]` 来表示过期。TTL 应加入少量 jitter，防止同一时间集中刷新。
...
`job_mcpt_<sha256(type, workspace_id, catalog_external_id, generation) 前 40 位>`
...
- 同一 `(workspace_id, endpoint_key, auth_scope_key)` 同时最多一个 active generation。
...
- DB 查询使用 principal 的内部 workspace ID，而不是只相信 path 或 `agentId`；
...
Agent Detail 没有 Session vault context，因此 MVP 只自动读取/刷新 `anonymous` scope。若匿名 probe 返回 401/403，显示 `auth_required`。未来带凭据发现必须由已授权的 Session/Deployment/Environment context 触发，`auth_scope_key` 使用 credential ID、版本和可见 scope 的 HMAC，绝不包含 token，也不能把一个用户的 catalog 暴露给另一个 scope。
...
- `internal/db/mcp_tool_catalogs.go`：workspace-scoped catalog CRUD、generation CAS、job lease/transition。
...
1. 新增 migration、DB catalog CRUD、generation 和 job 数据访问。
...
- workspace/org 隔离、相同 URL 跨 workspace 不共享、相同 workspace endpoint 复用；
...
- 是否增加 hard-stale 截止时间，以及是否按 workspace 配置；
