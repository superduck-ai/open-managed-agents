...
func (d *DB) GetFilestoreEntry(ctx context.Context, workspaceUUID, filesystemUUID, entryPath string) (FilestoreEntry, error) {
...
// ListFilestoreEntriesPage 以 (path, uuid) 为稳定排序键执行键集分页。
// 过期或软删除节点不会出现在结果中。
