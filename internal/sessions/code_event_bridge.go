...
func (h *Handler) appendAndBroadcastInternal(r *http.Request, sessionID string, events []db.SessionEvent) {
	...
	if err != nil {
		slog.Error("append internal session events", "session_id", sessionID, "error", err)
		return
	}
...
func (h *Handler) PublishCodeSessionEvents(ctx context.Context, codeSession db.CodeSession, payloads []json.RawMessage) error {
	...
	for _, raw := range payloads {
		if maevents.IsStreamDelta(rawSessionEventType(raw)) {
			...
			if err != nil {
				slog.Warn("skip code session stream delta", "session_id", session.ExternalID, "code_session_id", codeSession.ExternalID, "error", err)
				continue
			}
		...
		if err != nil {
			slog.Warn("skip code session event", "session_id", session.ExternalID, "code_session_id", codeSession.ExternalID, "error", err)
			continue
		}
