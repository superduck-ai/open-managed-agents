...
func (d *DB) MakeFilestoreDirectory(ctx context.Context, input MakeFilestoreDirectoryInput) (SessionResourceFile, error) {
	...
	return withFilestoreNamespaceMutation(
		ctx,
		d.mapperDB,
		input.WorkspaceUUID,
		input.FilesystemUUID,
		func(tx yourbatis.Executor, filesystem FilestoreFilesystem) (SessionResourceFile, error) {
			if input.Path == "/" {
			...
			for _, directoryPath := range paths {
				var err error
				directory, err = ensureFilestoreDirectoryTx(ctx, tx, filesystem, directoryPath, input.Now)
				if err != nil {
					return SessionResourceFile{}, err
				}
			}
...
func (d *DB) PutFilestoreFile(ctx context.Context, input PutFilestoreFileInput) (FilestoreMutationResult, error) {
	...
	return withFilestoreNamespaceMutation(
		...
		input.WorkspaceUUID,
		input.FilesystemUUID,
		func(tx yourbatis.Executor, filesystem FilestoreFilesystem) (FilestoreMutationResult, error) {
			return putFilestoreFileTx(ctx, tx, filesystem, putFilestoreFileTxInput{
		...
		func(tx yourbatis.Executor, filesystem FilestoreFilesystem) (FilestoreMutationResult, error) {
			...
			if found {
				...
				if err := retireSessionResourceFileTx(
					ctx, tx, input.WorkspaceUUID, destination.UUID, input.Now,
				); err != nil {
					return FilestoreMutationResult{}, err
				}
				...
				if removedBytes > 0 {
					if err := applyWorkspaceStorageDeltaTx(
						ctx, tx, input.WorkspaceUUID, 0, -removedBytes, 0,
					); err != nil {
						return FilestoreMutationResult{}, err
					}
				}
...
func (d *DB) MoveFilestoreDirectory(ctx context.Context, input MoveFilestoreDirectoryInput) (FilestoreMutationResult, error) {
	...
	return withFilestoreNamespaceMutation(
		...
		func(tx yourbatis.Executor, filesystem FilestoreFilesystem) (FilestoreMutationResult, error) {
			...
			maxMovedPathBytes, err := resourceMapper.MaxMovedPathBytes(ctx, moveParams)
			if err != nil {
				return FilestoreMutationResult{}, err
			}
			...
			containsInput, err := resourceMapper.SubtreeContainsInput(ctx, moveParams)
			if err != nil {
				return FilestoreMutationResult{}, err
			}
			if containsInput {
			...
			_, conflictFound, err := resourceMapper.FindMoveConflict(ctx, moveParams)
			if err != nil {
				return FilestoreMutationResult{}, err
			}
			if conflictFound {
			...
			if retiredBytes > 0 {
				if err := applyWorkspaceStorageDeltaTx(ctx, tx, input.WorkspaceUUID, 0, -retiredBytes, 0); err != nil {
					return FilestoreMutationResult{}, err
				}
			}
...
func (d *DB) RemoveFilestoreFile(ctx context.Context, input RemoveSessionResourceFileInput) (FilestoreMutationResult, error) {
	...
	return withFilestoreNamespaceMutation(
		...
		func(tx yourbatis.Executor, filesystem FilestoreFilesystem) (FilestoreMutationResult, error) {
			...
			if err := retireSessionResourceFileTx(ctx, tx, input.WorkspaceUUID, entry.UUID, input.Now); err != nil {
				return FilestoreMutationResult{}, err
			}
...
func (d *DB) RemoveFilestoreDirectory(ctx context.Context, input RemoveFilestoreDirectoryInput) (FilestoreMutationResult, error) {
	...
	return withFilestoreNamespaceMutation(
		...
		func(tx yourbatis.Executor, filesystem FilestoreFilesystem) (FilestoreMutationResult, error) {
			...
			resourceMapper := NewSessionResourceMapper(tx)
			childCount, err := resourceMapper.CountDirectoryChildren(ctx, subtreeParams)
			if err != nil {
				return FilestoreMutationResult{}, err
			}
			...
			containsInput, err := resourceMapper.SubtreeContainsMountedInput(ctx, subtreeParams)
			if err != nil {
				return FilestoreMutationResult{}, err
			}
			if containsInput {
			...
			if removedBytes > 0 {
				if err := applyWorkspaceStorageDeltaTx(ctx, tx, input.WorkspaceUUID, 0, -removedBytes, 0); err != nil {
					return FilestoreMutationResult{}, err
				}
			}
