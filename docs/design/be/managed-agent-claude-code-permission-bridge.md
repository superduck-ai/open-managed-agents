...
2. 将该阻塞请求投影成 Managed Agents public event 契约：
   ...
   - `stop_reason.event_ids` 包含阻塞 tool use id。
3. 等待 API client 发送：
...
4. 收到 confirmation 后，server 查找对应 pending `can_use_tool` 请求，生成 Claude Code inbound `control_response`：
   - `result=allow` -> `behavior=allow`
   - `result=deny` -> `behavior=deny`，携带 `deny_message`
5. `user.tool_confirmation` 本身保留为 public session event，用于审计和 Session Detail 只读回放。
...
- Session events 接收 `user.tool_confirmation` 后，补齐到 Claude Code `control_response` 的路由。
...
- `always_ask` tool call 暂停后，`user.tool_confirmation(result=allow)` 会生成 Claude Code allow response。
- `user.tool_confirmation(result=deny, deny_message=...)` 会生成 Claude Code deny response。
- confirmation 找不到 pending tool use 时返回 Anthropic-compatible validation error。
- subagent cross-post 的 permission request 能通过 `tool_use_id` 路由回原 code session。
...
- session 暂停并公开等待确认。
- 客户端发送 `user.tool_confirmation` 后，Claude Code 继续执行。
