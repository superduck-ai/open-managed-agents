---
title: "Adding files"
sidebarTitle: "Attach and download files"
...
First, upload a file using the [Files API](/docs/en/api/files/upload-file):
...
With the preceding `mount_path`, the agent reads the file at `/mnt/session/uploads/data.csv` (see [File paths](/docs/en/files#file-paths)).

A new `file_id` is created that references the instance of the file in the session. These copies do not count against your [storage limits](/docs/en/api/files/list-files).
...
Use the [Files API](/docs/en/api/files/list-files) to list files scoped to a session and download them.
