...
sidebarTitle: "Scheduled deployments"
description: "Create and manage deployments with the OMA API: run an agent on a recurring cron schedule and inspect its run history."
---

A **scheduled deployment** allows an [agent](/docs/en/agent-setup) to start [sessions](/docs/en/sessions) autonomously, enabling task completion over a predictable cadence. You create and manage deployments with the OMA Deployments API.
...
When creating a deployment, you pass the [session configurations](/docs/en/sessions) required for execution, in addition to a `schedule`.

* Deployments require [agent configuration](/docs/en/agent-setup) and [environment configuration](/docs/en/environments), and optionally accept [files](/docs/en/files), [GitHub](/docs/en/github), [memory stores](/docs/en/memory), and [vaults](/docs/en/vaults).
...
A maximum of **1,000 scheduled deployments** is supported per organization. Contact OMA support if you need more.

See the [Create Deployment reference](/docs/en/api/deployments/create-deployment) for full parameters and response schema.

### Cron and timezone semantics

* **Expression:** Standard POSIX cron (`minute hour day-of-month month day-of-week`). You can generate and validate these cron expressions in the OMA Console.
...
Pass the optional `budget` object when you create or update the deployment. It takes the same shape as a [session budget](/docs/en/budgets). The deployment copies the cap onto each session it starts, so the budget bounds every run separately rather than acting as a cumulative ceiling across runs: a deployment with a `"2000"` cap can spend up to about $20 on every run.

A session started by the deployment behaves exactly like any other budgeted session: it pauses with `budget_reached` when its own list cost [reaches the cap](/docs/en/budgets#when-a-session-reaches-its-budget). Changing the deployment's budget applies to runs started afterward; a session already running keeps the cap it started with, which you can [change through the session itself](/docs/en/session-operations#updating-the-session-budget). Unlike a session budget, a deployment's budget can be removed with `"budget": null` and set again later.
...
Successful deployments generate active sessions, and a successful deployment run contains the associated `session_id`. To follow a session's lifecycle, track the session events through the [event stream](/docs/en/events-and-streaming) or [webhooks](/docs/en/webhooks). Deployment lifecycle changes and the outcome of each scheduled run are also delivered as webhook events, listed in the Deployment events and Deployment run events tabs of [Supported event types](/docs/en/webhooks#supported-event-types).
...
To retrieve a single run by ID, call [`GET /v1/deployment_runs/{deployment_run_id}`](/docs/en/api/deployment-runs/get-deployment-run). A [`deployment_run` webhook event](/docs/en/webhooks#supported-event-types) carries the run ID as its `data.id`.

## Managing deployment lifecycle

Each lifecycle change emits a [webhook event](/docs/en/webhooks#supported-event-types), so you can react to a paused, unpaused, or archived deployment without polling; see the Deployment events tab.
...
To run a deployment outside its schedule, call the [`run` endpoint](/docs/en/api/deployments/run-deployment-now). This creates a session immediately and writes a deployment run with `trigger_context.type: "manual"`. This allows you to test a deployment before committing to the schedule.
