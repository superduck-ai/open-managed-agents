...
import { sessionNullableProcessedAt, sessionStableEventId, sessionThreadIsArchived, sessionThreadIsChild } from '../api';
import { entityAgentId, entityStatusLabel, entityVaultIds } from '../resources/ManagedResources';
import { type DisplayEventEntry, type DisplayEventType, type I18nMsg, type IdleGapEntry, type QueuedBoundaryEntry, type QuickstartSessionEvent, type SessionApiResponse, type SessionDetailLane, type SessionDetailLaneState, type SessionDetailSummaryChip, type SessionEventListEntry, type SessionEventUsage, type SessionResourceApiResponse, type SessionThreadApiResponse, type SessionTimelineItem, type SessionTimelineLane, type SessionTraceFilterOption, type SessionTraceView, type ToolBatchEntry, type ToolCallEntry } from '../types';
...
import { sessionEventSummary, sessionEventTimestamp, sessionEventType, sessionTraceFilterValue } from './sessionTraceModel';
...
export function buildSessionDetailSummary(
  ...
  if (vaults.length) {
    chips.push({ key: 'vaults', icon: LockKeyhole, value: vaults.length === 1 ? vaults[0] : msg('managedAgents.sessions.detail.vaultCount', '{count} vaults', { count: vaults.length }) });
  }
  chips.push({ key: 'created', icon: CalendarClock, value: formatRelativeFromNow(session.created_at, formatters, msg) });
...
export function buildSessionDetailFilterOptions(entries: SessionEventListEntry[], view: SessionTraceView, msg: I18nMsg): SessionTraceFilterOption[] {
  if (view === 'transcript') {
...
export function buildSessionDetailLaneState(threads: SessionThreadApiResponse[], msg: I18nMsg, showArchivedLanes: boolean): SessionDetailLaneState {
  ...
  const lanes: SessionDetailLane[] = [
    { id: SESSION_MAIN_LANE_ID, label: msg('managedAgents.sessions.detail.orchestrator', 'Orchestrator'), group: 'Main', isMain: true }
  ];
...
export function sessionTimelineItemFromEntry(entry: SessionEventListEntry): SessionTimelineItem | null {
  ...
  if (entry.kind === 'tool_call') {
    ...
    const durationMs = entry.bracketStartMs ? Math.max(0, entry.processedAtMs - entry.bracketStartMs + executionMs) : executionMs;
    return {
      id: entry.id,
  ...
  if (entry.kind === 'tool_batch') {
    ...
    const executionMs = Math.max(0, entry.executionMs ?? 0);
    const durationMs = entry.bracketStartMs ? Math.max(0, entry.processedAtMs - entry.bracketStartMs + executionMs) : executionMs;
    const label = entry.toolCounts.length === 1 ? entry.toolCounts[0]?.name ?? 'Tool' : `${entry.calls.length} parallel calls`;
    return {
      id: entry.id,
      rowId: entry.traceEntry.id,
      type: 'tool_use',
...
export function sessionDetailEventCopyPayload(entries: SessionEventListEntry[], view: SessionTraceView) {
  const selectableEntries = entries.filter((entry): entry is Exclude<SessionEventListEntry, IdleGapEntry | QueuedBoundaryEntry> => 'traceEntry' in entry);
  if (view === 'debug') {
    return JSON.stringify(selectableEntries.map((entry) => entry.event), null, 2);
  }
  return selectableEntries
    .map((entry) => {
      if (entry.kind === 'tool_batch') {
        const summary = entry.calls.map((call) => `${call.name}: ${call.inputPreview || call.displayEvent.content}`).join('; ');
        return `[${entry.relativeTime}] Tool batch: ${summary}`;
      }
      const label = entry.traceEntry.label || sessionEventType(entry.event);
      const text = entry.traceEntry.displayText || entry.traceEntry.preview || entry.displayEvent.content || sessionEventSummary(entry.event);
...
export function sessionResourcesLabel(resources: SessionResourceApiResponse[], msg: I18nMsg) {
  ...
  const fileCount = resources.filter((resource) => String(resource.type || resource.resource_type || '').includes('file')).length;
  if (fileCount > 0 && fileCount === resources.length) {
    return msg('managedAgents.sessions.detail.fileCount', '{count, plural, one {# file} other {# files}}', { count: fileCount });
  }
  return msg('managedAgents.sessions.detail.resourceCount', '{count, plural, one {# resource} other {# resources}}', { count: resources.length });
}
...
export function sessionThreadBaseName(thread: SessionThreadApiResponse) {
  ...
  const candidates = [record.name, record.title, record.role, record.label, metadata?.name, metadata?.role, agent?.name, agent?.id];
...
export function sessionThreadFallbackLabel(thread: SessionThreadApiResponse, index: number, msg: I18nMsg) {
  return msg('managedAgents.sessions.detail.agentLaneFallback', 'Agent {index}', { index }) || compactEntityId(thread.id || `thread-${index}`);
}
...
export function sessionEventThreadId(event: QuickstartSessionEvent) {
  ...
  return candidates.find((value): value is string => typeof value === 'string' && value.trim().length > 0)?.trim() ?? SESSION_MAIN_LANE_ID;
}

export function nearestSessionEventEntry<T extends SessionEventListEntry>(entries: T[], processedAtMs: number): T | null {
  if (!entries.length) {
    return null;
  }
...
export function sessionElapsedMs(session: SessionApiResponse, events: QuickstartSessionEvent[]) {
  ...
  const end = sessionStatusIsLive(session.status) && !session.archived_at ? Date.now() : eventEnd || Date.parse(session.updated_at);
...
export function extractSessionEventUsage(event: QuickstartSessionEvent): SessionEventUsage {
  ...
  const modelInput = modelUsage ? numericValueFromKeys(modelUsage, ['input_tokens', 'inputTokens', 'tokens_in', 'tokensIn', 'input']) : 0;
  const modelOutput = modelUsage ? numericValueFromKeys(modelUsage, ['output_tokens', 'outputTokens', 'tokens_out', 'tokensOut', 'output']) : 0;
  const modelCacheRead = modelUsage ? numericValueFromKeys(modelUsage, ['cache_read_input_tokens', 'cacheReadInputTokens', 'cache_read_tokens', 'cacheReadTokens', 'cache_read', 'cacheRead']) : 0;
  const modelCacheCreation = modelUsage ? numericValueFromKeys(modelUsage, ['cache_creation_input_tokens', 'cacheCreationInputTokens', 'cache_creation_tokens', 'cacheCreationTokens', 'cache_creation', 'cacheCreation']) : 0;
  const inputTokens = numericValueFromKeys(usage, ['input_tokens', 'inputTokens', 'tokens_in', 'tokensIn', 'input']) || modelInput || 0;
  const outputTokens = numericValueFromKeys(usage, ['output_tokens', 'outputTokens', 'tokens_out', 'tokensOut', 'output']) || modelOutput || 0;
  const cacheReadInputTokens = numericValueFromKeys(usage, ['cache_read_input_tokens', 'cacheReadInputTokens', 'cache_read_tokens', 'cacheReadTokens', 'cache_read', 'cacheRead']) || modelCacheRead || 0;
  const cacheCreationInputTokens = numericValueFromKeys(usage, ['cache_creation_input_tokens', 'cacheCreationInputTokens', 'cache_creation_tokens', 'cacheCreationTokens', 'cache_creation', 'cacheCreation']) || modelCacheCreation || 0;
  return {
    input: inputTokens + cacheReadInputTokens + cacheCreationInputTokens,
    output: outputTokens,
    input_tokens: inputTokens,
...
export function aggregateSessionModelUsage(value: unknown) {
  ...
  const directOutput = numericValueFromKeys(record, ['output_tokens', 'outputTokens', 'tokens_out', 'tokensOut', 'output']);
  const directCacheRead = numericValueFromKeys(record, ['cache_read_input_tokens', 'cacheReadInputTokens', 'cache_read_tokens', 'cacheReadTokens', 'cache_read', 'cacheRead']);
  const directCacheCreation = numericValueFromKeys(record, ['cache_creation_input_tokens', 'cacheCreationInputTokens', 'cache_creation_tokens', 'cacheCreationTokens', 'cache_creation', 'cacheCreation']);
  ...
  Object.values(record).forEach((modelValue) => {
    ...
    cacheRead += numericValueFromKeys(usage, ['cache_read_input_tokens', 'cacheReadInputTokens', 'cache_read_tokens', 'cacheReadTokens', 'cache_read', 'cacheRead']);
    cacheCreation += numericValueFromKeys(usage, ['cache_creation_input_tokens', 'cacheCreationInputTokens', 'cache_creation_tokens', 'cacheCreationTokens', 'cache_creation', 'cacheCreation']);
  });
...
export function sessionEventDurationMs(event: QuickstartSessionEvent) {
  return numericValueFromKeys(event, ['duration_ms', 'elapsed_ms', 'latency_ms', 'run_time_ms', 'processing_duration_ms']);
}
...
export function sessionTokenUsageTitle(usage: SessionEventUsage, formatters: ReturnType<typeof useFormatters>, msg: I18nMsg) {
  const rows = [
...
export function formatSessionDuration(ms: number, formatters: ReturnType<typeof useFormatters>, msg: I18nMsg) {
  ...
  if (secondsFloat < 60) {
    return msg('managedAgents.sessions.detail.durationSeconds', '{seconds}s', {
      seconds: formatters.number(Number(secondsFloat.toFixed(1)), { minimumFractionDigits: 1, maximumFractionDigits: 1 })
    });
  }
