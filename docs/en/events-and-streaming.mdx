...
sidebarTitle: "Session event stream"
description: "Send events, stream responses, and interrupt or redirect your session mid-execution."
---

Communication with Open Managed Agents is event-based. You send user events to the agent, and receive agent and session events back to track status.
...
* **Session events**, **span events**, and **agent events** are sent to you for observability into your session state and agent progress. Stream connections that opt in also receive [event deltas](/docs/en/events-and-streaming#event-deltas).

Session, span, agent, user, and system event type strings follow a `{domain}.{action}` naming convention. The stream-only delta preview events (`event_start`, `event_delta`) are the exception. See [Event types](/docs/en/reference#event-types) in the reference for the full catalog.
...
<Tabs>
  ...
  <Tab title="Listing past events">
    ...
    <CodeGroup>
      ...
      ```php PHP
      // In PHP, pass the types you want on EventListParams; see the OMA PHP SDK.
      ```
...
In the manual pattern, treat the preview as a scratch buffer and the buffered event as the record. Key the buffer by `(event_id, index)`. Reconcile per model request: a turn opens with a single `session.status_running` event, then on a turn that completes normally each model request produces, in order, `span.model_request_start`, `event_start`, the `event_delta` events, the buffered `agent.message`, and finally [`span.model_request_end`](/docs/en/reference#event-types) (in the Span events tab). On the wire, this is the previewed portion of that sequence, interleaved with the connection's other buffered events:
...
The preview events themselves don't change. `event_start` and `event_delta` have the same shape on a thread stream as on the session-level stream, and the [accumulate and reconcile](/docs/en/events-and-streaming#accumulate-and-reconcile) pattern applies as written. The one adjustment is bookkeeping: run one accumulator instance per stream connection.
...
The read loop exits on [`session.thread_status_idle`](/docs/en/reference#event-types), the event emitted when the session thread's turn finishes and the thread goes idle.
...
When the agent invokes a [custom tool](/docs/en/tools#custom-tools):
...
When a [permission policy](/docs/en/permission-policies) requires confirmation before a tool executes:
...
* **Troubleshoot previews:** If a stream that opts in to event deltas doesn't behave as you expect, see [Troubleshoot previews](/docs/en/events-and-streaming#troubleshoot-previews)
