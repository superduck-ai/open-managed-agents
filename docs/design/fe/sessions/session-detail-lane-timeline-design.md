...
页头、检查面板和费用曲线共用最新 `session.usage` 快照；实时、刷新和重连均按同一规则取值。快照完整替换旧指标，缺失显示“—”，显式零保留为零；省略费用时不回退旧费用，费用下调也原样展示。没有 usage 事件时兼容读取 Session GET；本层不负责从 raw result 推算累计用量或执行预算。
