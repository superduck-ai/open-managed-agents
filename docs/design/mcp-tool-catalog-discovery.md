...
当前已实现 migration、持久 catalog、generation 去重、异步 worker、Agent create/update best-effort 预热、Detail lazy ensure、Console GET/refresh API，以及前端状态展示与手动刷新。带凭据 auth scope、runtime observation、`tools/list_changed`、legacy SSE、按 endpoint 的速率限制和网络级地址策略仍属于后续范围；当前 probe 不检查 DNS/IP 是否指向本机、私网或特殊地址，不能把本 MVP 的 anonymous catalog 当作受限网络执行环境或运行时完整能力清单。
...
该 migration 不修改已应用 migration 或 `internal/db/schema.go`，且不创建 PostgreSQL foreign key。
...
`endpoint_url` 与 Agent 配置处于同一数据库安全边界，供 worker 请求使用，但它仍必须是 credential-free URL。anonymous MVP 拒绝所有带 query 的 endpoint，不会静默剥离参数后探测，因为这会改变 endpoint 语义；URL userinfo 同样始终拒绝。
...
1. 已完成 migration、organization/workspace-scoped DB catalog CRUD、generation、retention 和 job 数据访问。
...
7. `MCP_DISCOVERY_ENABLED` 控制发现功能；生产默认关闭，显式开启时必须提供 `MCP_DISCOVERY_HMAC_KEY`。开发默认开启。
