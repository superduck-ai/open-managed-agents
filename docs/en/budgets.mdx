...
sidebarTitle: "Session budgets"
description: "Cap a session's spend with a hard dollar budget enforced at public list rates."
---

<Warning>
  The current OMA release does not yet enforce hard dollar-denominated session budgets. This page documents the upstream-compatible target contract; operators should currently control cost with model quotas, infrastructure limits, and external billing alerts.
</Warning>

A session budget is an optional hard spend ceiling you set when you [create a session](/docs/en/sessions). The platform continuously prices everything the session consumes at public list rates (the session's **list cost**) and stops issuing new model requests once that cost reaches the budget. The request in flight when the cap is crossed still finishes, so the final list cost can land [a fraction past the budget](/docs/en/budgets#when-a-session-reaches-its-budget). A session at its budget pauses and goes [idle](/docs/en/session-operations#session-statuses) rather than terminating; changing or removing the budget resumes its work automatically. Deployments accept the same budget and apply it to each session they start; see [Budgets on deployments](/docs/en/budgets#budgets-on-deployments).
...
<CodeGroup defaultLanguage="CLI">
  ```bash cURL
  session=$(curl -sS --fail-with-body http://localhost:38080/v1/sessions \
...
A budget can only be attached when the session is created. Adding a budget to an existing session that doesn't have one is rejected with a 400 error. A budgeted session's cap can be [changed](/docs/en/budgets#change-the-budget) or [removed](/docs/en/budgets#remove-the-budget) at any time.
...
A session that reaches its budget goes idle with a `stop_reason` of `budget_reached`; it is not terminated, and its history and sandbox are preserved like any other idle session's. On the [event stream](/docs/en/events-and-streaming) you'll see, in order:

1. A `session.thread_status_idle` event with a `stop_reason` of `budget_reached` as each thread pauses.
2. A [`session.usage`](/docs/en/budgets#monitor-spend) event with the session's cumulative usage and list cost.
...
For reading usage from the stream and the session object, see [Tracking usage](/docs/en/events-and-streaming#tracking-usage).

## Budgets in multiagent sessions

A [multiagent](/docs/en/multiagent-orchestration) session has a single budget shared across all of its threads; there are no per-thread caps. Each thread's consumption is priced at its own served model, and threads pause independently as the shared cap is reached. [Advisor](/docs/en/multiagent-orchestration#give-the-session-an-advisor) consultations count against the same budget, priced at the advisor model's rates. One thread can pause at `budget_reached` while another finishes its in-flight request.

A pending ask outranks the cap: a session with one thread waiting on `requires_action` and another paused at `budget_reached` reports `requires_action` at the session level. The pending request still needs an answer, and answering it is a [settle event](/docs/en/budgets#events-accepted-at-the-cap) the budget doesn't block.

## Budgets on deployments

A [deployment](/docs/en/scheduled-deployments) accepts the same `budget` object when you create or update it:
...
| Condition                                                                                                                                                                                                                                   | Status |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| A work-starting event (for example, `user.message`) is sent while the session is at or over its budget; the error names the [accepted settle events](/docs/en/budgets#events-accepted-at-the-cap) | 400    |
...
| A budgeted create references a model with [no public list price](/docs/en/budgets#models-without-a-list-price)                                                                                    | 400    |
