...
`environment-manager` 的 `auth[type=anthropic_oauth]` 使用 lifecycle-bound OAuth-compatible token；`auth[type=session_ingress]` 使用自包含的 `sk-ant-si-<JWT>`。前者只访问 `/v1/messages`，后者供 worker、relay 与 upstream proxy 使用。启动 payload 不再包含 `auth[type=anthropic_api]` 或 `CLAUDE_CODE_SESSION_ACCESS_TOKEN`，避免环境变量遮蔽 WebSocket FD。Runner 创建 Cloud Session Sandbox 并完成 Packages 后，先 heartbeat Environment Work 并确认 Session 仍可启动，再等待固定 rclone-filestore 五挂载 ready 并最多重试三次删除临时 Token 配置；其中 `/uploads` 整体只读挂载到 `/mnt/session/uploads`，`/skills` 整体只读挂载到 `/root/.claude/skills`，都不执行逐文件 projection。随后 Runner 把 Sandbox 标记为 `running`，并在一个数据库事务中创建 local Code Session、固化初始 events 并把 runtime metadata 发布到 Session 和 Environment Work；事务成功后才通过 E2B 后台进程 API 启动 environment-manager、按 PID 直接发送并关闭 stdin。environment-manager 启动失败时 Runner 立即终止该 Code Session，并只撤销仍指向该 identity 的 runtime metadata。environment-manager 在启动 Claude 前 register CCR worker。work heartbeat 只维护 environment 租约，不参与 code-session token 鉴权。payload 不写入沙箱文件系统，发送或关闭失败时终止未完整初始化的进程。
