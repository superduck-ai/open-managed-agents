...
graceful stop 同样采用 best-effort provider 清理：即使把 `environment_sandboxes.state` 写成 `stopping` 失败，runner 仍必须尝试 Kill 已知的 provider Sandbox ID；Kill 成功后继续尝试写入 `stopped` 并强制停止 Environment Work，最后通过 joined error 返回所有持久化失败，避免单个数据库错误阻止其余清理。测试失败信息只记录 launch 数量、sandbox ID、命令或 payload 字节数，不输出 Environment Manager stdin、完整 launch 对象或凭证。
