...
- 需要在整库恢复、租户搬迁、跨库合并或部分数据导入后保持含义的持久化跨表引用，使用被引用资源的 `*_uuid`，不要保存另一张表可重新生成的 identity 主键；例如 `filestore_filesystems` 的租户、会话和创建者归属，以及 `session_resources.file_uuid` 与 `skill_version_uuid` 都使用对应 UUID。
