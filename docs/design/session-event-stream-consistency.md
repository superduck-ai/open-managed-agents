...
原版 CLI 的请求身份、块索引和源 UUID 接收凭据在 OMA 入口规范化；migration 00060 保存接收凭据。跨批次重试复用原身份，凭据、请求边界与完整事件同事务提交，提交后发布预览。关联依赖原版上传顺序及 UUID 重试合同。

SSE 仅接收本连接已观察到的当前请求预览，匹配的结束事件关闭预览，旧请求不影响新请求。前端以 ID 去重并保留微秒顺序，每次建流后补齐完整历史快照；获取失败不提交部分缓存，重连和刷新都从完整事件恢复。

## 错误与系统事件

公开 error 在输入边界校验必需字段，保留未知附加字段与数值精度；非法输入返回固定 400 文案并回滚整批，冲突返回 409。历史兼容读取未知错误类别。

worker 重试与压缩映射为专属公共进度事件，内部初始化和诊断不再进入对话。明确成功的 result 使用线程级 `end_turn`；该映射不修复瞬态 idle 的提前结束，也不推断重试耗尽或失败收尾。

## 用量快照

canonical `session.usage` 在事件事务内完整替换 Session 用量；旧 ID 重试不回退投影，非法内容或冲突回滚整批。前端页头、检查面板与曲线读取最新快照，区分未知与零；合法费用下调正常显示，新快照省略费用时不沿用旧值。

本实现不从 raw result 推算累计用量，不执行预算，也不保证 usage 紧邻 idle。

## 升级与边界

排空旧 writer 后执行迁移并切换服务，避免旧实例不推进处理时钟。旧记录保留原时间；旧游标失效时重新读取。回滚前先停写并评估新增状态，不直接启用旧 writer。

preview 仍为 best-effort，完整事件才是恢复依据。
