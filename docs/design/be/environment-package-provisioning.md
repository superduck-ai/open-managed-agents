...
sequenceDiagram
    ...
    alt 创建/安装期间收到 stop 请求
        ...
        Runner->>CodeSession: 创建 active session 并签发 sandbox 凭证
        Runner->>Runner: 提交 Session/Work runtime metadata
...
graceful stop 同样采用 best-effort provider 清理：即使把 `environment_sandboxes.state` 写成 `stopping` 失败，runner 仍必须尝试 Kill 已知的 provider Sandbox ID，并在 Kill 后返回原始状态持久化错误，避免数据库故障阻止外部资源回收。测试失败信息只记录 launch 数量、sandbox ID、命令或 payload 字节数，不输出 Environment Manager stdin、完整 launch 对象或凭证。
...
- `tests/environments_runner_cloud_test.go`：manifest/provisioner/startup 写入顺序、固定命令不含 spec、provisioning 失败或 stop 不创建 code session、状态写失败仍 Kill，且失败日志不输出 launch stdin；
