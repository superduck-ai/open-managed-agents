...
`Managed Agents`。交互层级参考 Claude Console 的资源列表、独立详情和 Agent MCP 选择流程，并使用 OMA
的 tunnel-client、Token、Channel、NATS Broker、Probe 与 Runtime Gateway；前端不暴露或模拟
...
Picker 以一个 Tunnel 作为一个候选项，不按 Channel 展开。选择后直接在 MCP 区域生成并滚动到内联配置卡片；
...
保存时使用 `mcp_servers` 与 `mcp_toolsets` API 字段：

| Channel | `mcp_server.name`               | URL                            |
| ------- | ------------------------------- | ------------------------------ |
| `main`  | `tunnel_<32-hex-id>.main`      | `/v1/mcp/{tunnelId}`           |
| 其他    | `tunnel_<32-hex-id>.{channel}` | `/v1/mcp/{tunnelId}/{channel}` |

为满足 MCP server name 不含 `__` 的约束，名称中的 Channel 后缀将 `_` 转为 `.`；Channel 本身不允许 `.`，因此此映射不会碰撞，实际 URL 仍使用原始 Channel。
...
已配置卡片使用同一 Channel 配置区。键盘输入先停留在本地编辑缓冲，只有 Apply 或选择有效实时建议后，才原子更新
`mcp_server.name`、`mcp_server.url` 和对应的 `mcp_toolset.mcp_server_name`；工具权限、配置顺序和其他 Agent 字段保持不变。
成功切换后清除旧 Channel 的临时发现结果，并在 Tunnel 已连接时对新 Channel 重新 Probe。Raw 配置按 canonical URL 回显。
...
待确认阻断、Channel 原子更新、重复校验、多 Channel 保存、添加或切换后自动发现、卡片手动刷新，以及
