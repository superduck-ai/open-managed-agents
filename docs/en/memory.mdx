...
Memory stores are attached in the session's `resources[]` array when the [session is created](/docs/en/sessions#creating-a-session). Unlike file resources, memory stores can only be attached at session creation time; adding or removing one from a running session is not supported.
...
See the [List memories reference](/docs/en/api/memories/list-memories) for full parameters and response schema.
...
See the [Retrieve a memory reference](/docs/en/api/memories/get-a-memory) for full parameters and response schema.

### Create a memory

`memories.create` creates a memory at a given `path`. Create does not overwrite; to change an existing memory, use [`memories.update`](/docs/en/memory#update-a-memory).
...
See the [Create a memory reference](/docs/en/api/memories/create-a-memory) for full parameters and response schema.
...
See the [Update a memory reference](/docs/en/api/memories/update-a-memory) for full parameters and response schema.
...
See the [Delete a memory reference](/docs/en/api/memories/delete-a-memory) for full parameters and response schema.
...
See the [List memory versions reference](/docs/en/api/memories/list-memory-versions) for full parameters and response schema.
...
See the [Retrieve a memory version reference](/docs/en/api/memories/get-a-memory-version) for full parameters and response schema.
...
See the [Redact a memory version reference](/docs/en/api/memories/redact-a-memory-version) for full parameters and response schema.

## Manage memory stores

In addition to [`create`](/docs/en/api/memory-stores/create-a-memory-store), memory stores support [`retrieve`](/docs/en/api/memory-stores/get-a-memory-store), [`update`](/docs/en/api/memory-stores/update-a-memory-store), [`list`](/docs/en/api/memory-stores/list-memory-stores), [`archive`](/docs/en/api/memory-stores/archive-a-memory-store), and [`delete`](/docs/en/api/memory-stores/delete-a-memory-store).
...
See the [List memory stores reference](/docs/en/api/memory-stores/list-memory-stores) for full parameters and response schema.
...
See the [Archive a memory store reference](/docs/en/api/memory-stores/archive-a-memory-store) for full parameters and response schema.

To permanently remove a store along with all of its memories and versions, use [`memory_stores.delete`](/docs/en/api/memory-stores/delete-a-memory-store).
...
* **Condense or prune before the store fills up.** Delete stale or redundant memories with `memories.delete`. You can also run a [dreaming session](/docs/en/dreams), which consolidates fragmented content into a separate new output store rather than modifying the original. Switch your sessions over to that output store, then archive or delete the original.
