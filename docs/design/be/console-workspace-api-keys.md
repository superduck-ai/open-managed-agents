...
空间解析成功后，还需针对 URL 中的真实目标空间调用统一授权服务，并检查开发或管理 API Key 的能力。Org Billing 继承计费权限，不能创建或管理 Key；普通空间显式提升为 Workspace Admin 后才具有相应能力。全组织 Key 列表仅 Org Admin 可访问。
...
Billing 提权和恢复规则见 [Billing 工作区权限](./billing-workspace-permissions.md)。
