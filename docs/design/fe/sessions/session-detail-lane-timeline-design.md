...
实时事件缓存与历史 API 按 `processed_at` 排序，保留微秒精度；同时间戳保留服务端写入顺序，禁止按随机 event ID 重排。全量历史同步按分页顺序校正并发到达的缓存事件，最终消息替换预览后采用其持久化到达位置。代理产生的
start/end 用 `request_id` 标识请求；end 用 `model_request_start_id` 配对，消息通过该引用或
end 的 `event_ids` 归属请求。必须同时满足 thread 相同，显式但找不到的引用不会回退到其他请求。
缺少引用的旧消息仍正常展示，但不推断模型请求归属；请求仅由关联的 end 闭合，不再使用 idle 等状态猜测请求结束。不修正历史中已经补造的时间事实。

start 到达且尚无消息时，minimap 直接显示进行中的模型请求区间；end 到达即固定宽度。
之后到达的最终消息继承该区间，不延长请求时长。没有最终消息的失败请求也保留已闭合的时间条。
end 清理它列出的未完成预览，并阻止这些预览的迟到片段重新出现；不会清理并发请求的预览。
消息替换预览只按相同 event ID，不再根据时间相同或只剩一个预览猜测。
