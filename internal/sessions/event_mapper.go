...
func (h *Handler) expandSessionStatusEvents(ctx context.Context, session db.Session, codeSessionID string, events []db.SessionEvent, payload map[string]any) ([]db.SessionEvent, error) {
	...
	if eventType == "session.status_running" || eventType == "session.status_idle" {
		...
		if eventType == "session.status_idle" {
			usage, err := sessionUsageEvent(events[0])
			if err != nil {
				return nil, err
			}
			return append([]db.SessionEvent{threadEvent, usage}, events...), nil
		}
	...
	if eventType == "session.thread_status_idle" || eventType == "session.thread_status_running" {
		...
		if eventType == "session.thread_status_idle" {
			usage, err := sessionUsageEvent(sessionEvent)
			if err != nil {
				return nil, err
			}
			return append(events, usage, sessionEvent), nil
		}
...
func sessionUsageEvent(idle db.SessionEvent) (db.SessionEvent, error) {
	event := idle
	event.UUID = uuid.NewV4().String()
	event.ExternalID += "_usage"
	event.EventType = "session.usage"
	event.StatusThreadID = ""
	var err error
	event.EncodeUsageSnapshot = func(usage json.RawMessage) (json.RawMessage, error) {
		return jsonv2.Marshal(map[string]any{"id": event.ExternalID, "type": event.EventType, "processed_at": event.ProcessedAt, "usage": usage, "budget": nil})
	}
	event.Payload, err = event.EncodeUsageSnapshot(json.RawMessage(`{}`))
	return event, err
}
