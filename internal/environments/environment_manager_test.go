...
func TestBuildEnvironmentManagerPayloadAndCommand(t *testing.T) {
	...
	payload, err := buildEnvironmentManagerV0Payload("cse_test", sessionIngressToken, oauthAccessToken, "/workspace/widgets", sessionConfig, cfg, nil)
	if err != nil {
...
func TestBuildEnvironmentManagerPayloadPreservesMCPConfig(t *testing.T) {
	...
	payload, err := buildEnvironmentManagerV0Payload("cse_test", "sk-ant-si-test-token", "sk-ant-oat01-test-token", "", sessionConfig, cfg, nil)
	if err != nil {
		t.Fatalf("build payload: %v", err)
	}
	var body map[string]any
