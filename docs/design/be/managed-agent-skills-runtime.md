...
Version UUID。迁移 `00049_add_session_resource_file_ownership.sql` 保持 Skill Archive ownership
为 `NULL`，并拒绝将 Skill File 作为 referenced Input Source。Resource + File 成为 Session 内唯一的 Skill 快照事实；schema 不创建
PostgreSQL 外键。
