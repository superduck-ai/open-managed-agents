...
2. 从合法 MCP URL 提取 host，与 Environment networking allowlist 合并并去重；只接受 provider 支持的 URL scheme，URL path、query、fragment、userinfo/credential 不进入 egress allowlist。
...
MCP 网络策略与上述 preparation/Resolve 顺序使用同一合同：runner 从 session 固定的 agent snapshot 读取 MCP server URL，只把受支持 URL 的 host 写入内存 metadata；provider `Resolve` 将这些 host 与 Environment 的 `networking.allowed_hosts` 合并、规范化并去重，使它们进入本次 limited-network `AllowOut`。该输入是受控的 host 集合，不会把任意 scheme、path、query、fragment 或 credential 扩散到网络策略。因为 agent snapshot 在 session 创建时已经固定，后续 agent 更新不会改变已启动或正在创建的 session egress。
