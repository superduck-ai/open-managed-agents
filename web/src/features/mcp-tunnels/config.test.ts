...
describe('MCP tunnel presentation helpers', () => {
  ...
  test('uses a stable channel suffix for every channel', () => {
    ...
    expect(tunnelChannelServerName(tunnelID, 'internal__tools')).toBe(`${tunnelID}.internal..tools`);
    expect(tunnelChannelServerName(tunnelID, 'main')).toBe(`${tunnelID}.main`);
    expect(tunnelChannelServerName(tunnelID, 'secondary')).toBe(`${tunnelID}.secondary`);
  });
