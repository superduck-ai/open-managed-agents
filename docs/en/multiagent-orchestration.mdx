...
sidebarTitle: "Multiagent orchestration"
...
All agents share the same sandbox, filesystem, and [vault credentials](/docs/en/vaults), but each agent runs in its own **session thread**, a context-isolated event stream with its own conversation history. The coordinator reports activity in the **primary thread** (which is the same as the session-level [event stream](/docs/en/events-and-streaming)); additional threads are spawned at runtime when the coordinator delegates work.
...
Each agent uses its own configuration: model, system prompt, tools, MCP servers, and skills. Session-level [agent configuration overrides](/docs/en/sessions#override-agent-configuration-for-a-session) are the exception; they apply to the coordinator and its `self` copies. Tools, MCP servers, and context are not shared.
...
When [defining your agent](/docs/en/agent-setup), set `multiagent` to declare the roster of agents the coordinator can delegate to:
...
* `{"type": "self"}` allows the coordinator to spawn copies of itself. If the session was created with [agent configuration overrides](/docs/en/sessions#override-agent-configuration-for-a-session), those overrides also apply to these copies; roster entries referenced by ID are unaffected.
* `{"type": "advisor", "model": "<model id>"}` gives the session's primary thread an advisor it can consult mid-turn. At most one advisor entry per roster. See [Give the session an advisor](/docs/en/multiagent-orchestration#give-the-session-an-advisor).

The coordinator's configuration, including its `multiagent.agents` roster, is snapshotted when the coordinator is created or updated. Referenced agents stay pinned to the versions resolved at that time and do not automatically pick up later updates to their definitions. To delegate to a newer version of a referenced agent, [update the coordinator](/docs/en/agent-setup#update-an-agent) so its roster references that version.
...
When agents pin an inference geography (`model.inference_geo` in the [agent definition](/docs/en/agent-setup)), the coordinator's pin and every roster member's pin must either all be set to the same value or all be unset. A mismatched roster is rejected with a 400 validation error, both when the agent is saved and when a [session-create override](/docs/en/sessions#override-agent-configuration-for-a-session) changes any of the pins.
...
The advisor model must meet a minimum capability bar, and the agent's own model must not be more capable than its advisor; models of equal capability can pair. An invalid pairing is rejected with a 400 validation error when the agent is saved. Valid pairings follow the advisor tool's [model compatibility](/docs/en/multiagent-orchestration#give-the-session-an-advisor) table.

The advisor is also available as a [server tool on the Messages API](/docs/en/multiagent-orchestration#give-the-session-an-advisor). The Managed Agents surface differs in configuration and delivery: the roster entry has no `max_uses`, `max_tokens`, or `caching` fields, and advice arrives through thread events rather than `advisor_tool_result` blocks.
...
Advisor threads are exempt from the concurrent-thread limit. They appear in the session's [thread list](/docs/en/multiagent-orchestration#threads) with `agent` set to the advisor form exactly as configured (`{"type": "advisor", "model": ...}`) and `parent_thread_id` set to the primary thread.
...
To remove the advisor, [update the agent](/docs/en/agent-setup#update-an-agent) with a roster that no longer includes the advisor entry. If the advisor is the roster's only entry, clear the roster entirely by setting `"multiagent": null`.
...
[Agent configuration overrides](/docs/en/sessions#override-agent-configuration-for-a-session) at session creation can replace the coordinator's MCP servers and those of its `self` copies.
...
A [session budget](/docs/en/budgets) is a single shared cap across all of a session's threads. As the cap is reached, threads pause independently, and each thread's cost is priced at the thread's own served model.

<Note>
  A maximum of 25 concurrent threads is supported. The coordinator can call multiple copies of a single agent in the roster, creating multiple threads associated with one `agent`. [Advisor](/docs/en/multiagent-orchestration#give-the-session-an-advisor) consultation threads are exempt from this limit.
</Note>
...
Advisor consultations emit these same thread events under the reserved name `anthropic.advisor` (as `agent_name` on the thread lifecycle events and `from_agent_name` on the advice delivery); see [Give the session an advisor](/docs/en/multiagent-orchestration#give-the-session-an-advisor) for the sequence.
...
If a subagent needs something from your client, such as [permission](/docs/en/events-and-streaming#tool-confirmation) to run an `always_ask` tool, or the [result of a custom tool](/docs/en/events-and-streaming#handling-custom-tool-calls), the event is cross-posted to the **primary thread** with `session_thread_id` identifying the originating session thread.
...
The following example extends the [tool confirmation handler](/docs/en/events-and-streaming#tool-confirmation) to route replies. The same pattern applies to `user.custom_tool_result`.
