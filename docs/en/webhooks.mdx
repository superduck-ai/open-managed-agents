...
sidebarTitle: "Subscribe to webhooks"
description: "Get notified when major events happen without polling."
---

Sessions are long-running interactions. While most real-time interactions happen through the [SSE event stream](/docs/en/events-and-streaming), webhooks notify you of major state changes.
...
<Tabs>
  <Tab title="Session events">
    ...
    | `session.budget_reached`           | The session reached its [budget](/docs/en/budgets) and paused. Fires at most once for each budget value you set; changing the budget arms it again.                                                                                                                                                                                                                                                                                            |
    ...
    | `session.thread_created`           | New [multiagent thread](/docs/en/multiagent-orchestration) opened: an additional agent called by the coordinator is starting work, or the session's [advisor](/docs/en/multiagent-orchestration#give-the-session-an-advisor) is being consulted.                                                                                                                                                     |
    | `session.thread_idled`             | An agent in a [multiagent interaction](/docs/en/multiagent-orchestration) is waiting for input.                                                                                                                                                                                                                                                                                                                                                |
    | `session.thread_terminated`        | A [multiagent thread](/docs/en/multiagent-orchestration) terminated, either because the thread was archived or because it exhausted its retries. A coordinator-spawned child that finishes its work goes `idle`, not `terminated` (an advisor thread terminates once its consultation completes). Fires for child threads only; the primary thread's end, including archiving the whole session, surfaces only as `session.status_terminated`. |
    | `session.outcome_evaluation_ended` | [Outcome evaluation](/docs/en/define-outcomes) for a single iteration completed.                                                                                                                                                                                                                                                                                                                                                               |
  ...
  <Tab title="Agent events">
    ...
    | `agent.updated`  | A [new version of the agent](/docs/en/agent-setup#update-an-agent) was published. Updates that do not create a new version do not trigger this event. |
  ...
  <Tab title="Deployment run events">
    | Event                      | Trigger                                                                                                                                                                                                                                                                                                                                                                   |
    | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | `deployment_run.started`   | A scheduled run started. Only scheduled runs emit `deployment_run` events; [manual runs](/docs/en/scheduled-deployments#trigger-a-manual-run) do not.                                                                                                                                                                           |
    | `deployment_run.succeeded` | A scheduled run created its session. The event carries the same `data.id` (the run ID) as the run's `deployment_run.started` event. To follow the session's work, subscribe to its session events (the Session events tab), or fetch the [deployment run](/docs/en/scheduled-deployments#deployment-runs) for its `session_id`. |
    | `deployment_run.failed`    | A scheduled run did not create a session. The event carries the same `data.id` as the run's `deployment_run.started` event. Fetch the [deployment run](/docs/en/scheduled-deployments#deployment-runs) for the error details.                                                                                                   |
  </Tab>
  ...
  <Tab title="Environment events">
    ...
    An environment's [work items](/docs/en/self-hosted-sandboxes) emit no webhook events.
  </Tab>

  <Tab title="Memory store events">
    | Event                   | Trigger                                                                                                                                                                                                                                                                                             |
    | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | `memory_store.created`  | Memory store created, either by you or by an OMA-operated process that clones one of your existing stores.                                                                                                                                                                                    |
    ...
    Individual [memories](/docs/en/memory) and memory versions emit no webhook events.
  </Tab>
</Tabs>

## Register an endpoint

Visit **Manage > Webhooks** in the OMA Console.
...
Parse the body, switch on `data.type`, and fetch the resource by ID. Return any `2xx` to acknowledge. Any other response counts against the endpoint: a `3xx` disables it immediately (redirects are never followed), while other failures are retried; see [Delivery behavior](/docs/en/webhooks#delivery-behavior) for the retry and auto-disable rules.
...
* **Auto-disable:** An endpoint is automatically set to `disabled` with a machine-readable `disabled_reason` in three cases:
  ...
  * The endpoint's URL resolves to a non-public IP address when OMA connects. This disables the endpoint immediately, with the reason `auto-disabled: endpoint URL resolved to an invalid address`.
