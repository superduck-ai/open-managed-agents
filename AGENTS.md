...
- logger 和 handler 统一在可执行程序的组装层通过 `internal/observability` 创建，并在启动早期调用 `slog.SetDefault`。业务包使用注入的 `*slog.Logger` 或默认 logger，不要各自创建 handler，也不要在启动完成后修改全局默认 logger。
