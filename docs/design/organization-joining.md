...
flowchart TD
    ...
    G -->|接受成功或拒绝后无剩余邀请| H[返回原组织首页，不切换组织]
    G -->|拒绝后仍有邀请| D
    D -->|稍后处理或返回| C
    D -->|直达页面且无邀请| H
```

根据用户实测 CMA 后确认的交互，独立邀请页接受成功后直接返回原组织首页，不切换到受邀组织；拒绝最后一份邀请也返回原组织首页，拒绝后仍有邀请则继续展示。直达邀请页且没有邀请时直接返回首页。原组织失效时沿用现有权限恢复机制。移除“已加入／进入组织”二次操作；新加入的组织从账号菜单主动切换。提供“稍后处理，返回控制台”，保留原组织和原目标页面；同次打开不反复重定向，刷新时重新检查。返回地址仅接受站内路径，排除 `/invites` 自循环和外部 URL。写入后刷新失败留在页面，重试只刷新，不重复提交接受或拒绝。

菜单对话框与独立页共用 `InvitationList`、错误映射、CSRF 和接口，不另建邀请状态机或 API。已处理邀请先取消在途旧查询并移出缓存，再刷新服务端列表与 bootstrap。

### 账号菜单与切换流程

账号菜单不再展示“当前工作区 / API 方案”单选项，避免与组织列表混淆；工作区通过左上角选择器展示和切换。

左下角账号菜单提供组织列表、当前组织与角色，以及带待处理数量的“组织邀请”入口。菜单邀请对话框处理后保留当前页面，不自动切换组织，也不提供“进入组织”二次按钮；仅从账号菜单选择组织时才执行切换。拒绝后邀请从待处理列表移除，重新加入需管理员发送新邀请。
