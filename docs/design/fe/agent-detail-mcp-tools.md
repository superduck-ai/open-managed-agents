...
当前定义展示 `bash`、`read`、`write`、`edit`、`glob`、`grep`、`web_fetch`。其中 `web_fetch` 对应 Session Sandbox 内 Claude Code 的 `WebFetch` 客户端工具；详情页不把 Messages API 的模型服务端同名工具或 `web_search` 混入该清单。
