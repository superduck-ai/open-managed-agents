...
func (entry *SessionNamespaceNode) ReferencesSourceFile() bool {
	return entry != nil && entry.ReferencedFileUUID != nil
}

// OwnedBytes 返回该节点计入 Session Owned File 的字节数。
// 只有普通 File 节点拥有对象；Input Resource 和 Skill Archive Resource
// 都只引用其他资源拥有的对象。
func (entry *SessionNamespaceNode) OwnedBytes() int64 {
	if entry == nil ||
		entry.Kind != SessionNamespaceNodeKindFile ||
