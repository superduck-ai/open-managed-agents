...
- Deployment Run 将已规范化 resources 一次解码为命名结构，并在创建 Session 前重新查询 File、Memory Store 等可变引用；Deployment 专属边界只解析模板与 secret、生成本次 Session Resource ID，并把 FileSpec 与查询到的文件记录交给共享 Session 创建计划。该计划与直接创建 Session 共用 Resource 写入和事件 File 绑定规则，最终继续复用同一个 Session 创建事务，不存在 Deployment 专属的 Session 物化系统。
