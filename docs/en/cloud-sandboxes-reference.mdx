...
sidebarTitle: "Cloud sandbox reference"
...
Cloud sandboxes run as isolated Linux containers on OMA-managed infrastructure. They come pre-installed with a comprehensive set of programming languages, databases, and utilities. The agent can use these immediately without any installation steps.
...
| Network          | API-created environments default to [`unrestricted` networking](/docs/en/environments#networking); sandboxes provisioned through the OMA Console default to `limited` |
